...
namespace BDServerSonic
{
    public partial class Final : Form
    {
        ...
        {
            dataGridView1.DataSource = ConexionPostgreSQL.EjecutaConsultaSelect("SELECT * FROM Final ORDER BY idFinal");
        }
        ...
        {
            int idFinal = (int)dataGridView1.SelectedRows[0].Cells[0].Value;
            consulta = "UPDATE Final SET  estatus = False WHERE idFinal =  " + idFinal.ToString();
            ConexionPostgreSQL.EjecutaConsulta(consulta);
            MostrarDatos();
        }
